...
local function registerNativeOptions()
    ...
    if opt then
        ...
        opt:addTickBox("Vehicle_LimitDriveZoom", "Prevent Extreme Auto-Zoom While Driving", (MPOptim.Config and MPOptim.Config.Get("Vehicle_LimitDriveZoom")) ~= false, "Prevents extreme 200 percent camera auto-zoom from quadrupling visible tile draw calls")
        opt:addTickBox("Vehicle_PreDrivePurge", "Pre-Drive RAM Purge on Vehicle Entry", (MPOptim.Config and MPOptim.Config.Get("Vehicle_PreDrivePurge")) ~= false, "Runs a quick memory cleanup when entering a vehicle")
        opt:addTickBox("Vehicle_ScaleLightingFPS", "Scale Dynamic Lighting While Driving", (MPOptim.Config and MPOptim.Config.Get("Vehicle_ScaleLightingFPS")) ~= false, "Reduces dynamic lighting update rate during high-speed travel")
